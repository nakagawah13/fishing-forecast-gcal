...
class TestLocation:
    ...
    def test_create_valid_location(self) -> None:
        """正常な地点情報を作成できること"""
        location = Location(id="tokyo_bay", name="東京湾", latitude=35.6762, longitude=139.6503)
    ...
    def test_empty_id_raises_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="id must not be empty"):
            Location(id="", name="東京湾", latitude=35.6762, longitude=139.6503)
    ...
    def test_empty_name_raises_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="name must not be empty"):
            Location(id="tokyo_bay", name="", latitude=35.6762, longitude=139.6503)
    ...
    def test_latitude_below_minus_90_raises_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="latitude must be between -90 and 90"):
            Location(id="tokyo_bay", name="東京湾", latitude=-91.0, longitude=139.6503)
    ...
    def test_longitude_below_minus_180_raises_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="longitude must be between -180 and 180"):
            Location(id="tokyo_bay", name="東京湾", latitude=35.6762, longitude=-181.0)
    ...
    def test_latitude_at_boundary_is_valid(self) -> None:
        ...
        location_south = Location(id="south_pole", name="南極", latitude=-90.0, longitude=0.0)
        assert location_south.latitude == -90.0

        location_north = Location(id="north_pole", name="北極", latitude=90.0, longitude=0.0)
        assert location_north.latitude == 90.0

    def test_longitude_at_boundary_is_valid(self) -> None:
        """経度の境界値（-180, 180）が有効であること"""
        location_west = Location(id="west_edge", name="西端", latitude=0.0, longitude=-180.0)
        assert location_west.longitude == -180.0

        location_east = Location(id="east_edge", name="東端", latitude=0.0, longitude=180.0)
