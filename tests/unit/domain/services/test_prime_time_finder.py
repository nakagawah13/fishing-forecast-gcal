...
class TestPrimeTimeFinder:
    ...
    def test_find_with_multiple_high_tides(self, finder: PrimeTimeFinder) -> None:
        """複数満潮: 06:00満潮, 18:00満潮 → 両方の時合いを返す"""
        events = [
            TideEvent(
        ...
        # 最初の満潮(06:00)から±2時間
        assert result[0] == (
            datetime(2026, 2, 8, 4, 0, tzinfo=UTC),
            datetime(2026, 2, 8, 8, 0, tzinfo=UTC),
        )
        # 2番目の満潮(18:00)から±2時間
        assert result[1] == (
            datetime(2026, 2, 8, 16, 0, tzinfo=UTC),
            datetime(2026, 2, 8, 20, 0, tzinfo=UTC),
        )
    ...
    def test_find_with_mixed_events(self, finder: PrimeTimeFinder) -> None:
        ...
        # 09:30の満潮から±2時間
        assert result[0] == (
            datetime(2026, 2, 8, 7, 30, tzinfo=UTC),
            datetime(2026, 2, 8, 11, 30, tzinfo=UTC),
        )
        # 22:00の満潮から±2時間
        assert result[1] == (
            datetime(2026, 2, 8, 20, 0, tzinfo=UTC),
