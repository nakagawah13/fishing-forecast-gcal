...
**潮位の基準面（重要）**:
- **本ツールの出力**: 観測基準面からの高さ（cm）
  - UTideは観測データの平均潮位（MSL: Mean Sea Level）を基準とする
  - `coef['mean']` に観測データの平均潮位が保存される
  - 予測値 = UTide出力（平均からの偏差） + `coef['mean']`（平均潮位）
  - 気象庁の観測データは「観測基準面からの高さ」で記録されている
  - 例: 東京の観測基準面は T.P.（東京湾平均海面）から -188.40cm
- **気象庁推算値の基準**: DL（略最低低潮面）
  - 気象庁の潮位表（推算値 suisan）は DL を 0cm とする
  - DL は「これ以上潮が引くことはほとんどない」面
  - 船舶の安全運航のための基準
  - 東京の場合、DL は T.P. から約 -100cm（推定）
- **基準面の差異**: 本ツールと気象庁推算値では約88cm のオフセットが存在（東京の場合）
  - 観測基準面（T.P. -188.40cm） - DL（T.P. -100cm） = -88.40cm
  - 本ツールの潮位値は気象庁推算値より約88cm高く表示される
  - **これは基準面の違いによるもので、予測誤差ではない**
- 詳細: [潮位データ：推算値と実測値の違い.md](./潮位データ：推算値と実測値の違い.md)
