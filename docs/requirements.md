...
- スマホのホーム画面にある Google カレンダーウィジェットで日々の予定を把握している
- ロック解除直後の一目確認に潮汐情報を統合したい
- タイドグラフアプリの多段タップと広告表示を避けたい
- MVP はホームグラウンド数カ所の潮汐表示から着手する
...
- OAuth 同意画面を Testing で運用する場合、トークン期限による再認証が必要になる可能性がある

## OAuth運用ルール
- MVPでは OAuth 同意画面は Testing を前提とし、トークン期限切れ時は再認証で対応
- 期限切れや認証失敗を検知した場合は、ログに警告を出し自動更新を停止する
- Production 化はMVP以降に検討し、必要であれば審査手続きを実施する
